...
# Fit the lasso model with all the other main effects with preprocessing and cv
bw_lasso = bw %>% 
...
```{r}
bw_2plm = lm(bwt ~ blength + gaweeks, data = bw)
bw_intlm = lm(bwt ~ (bhead + blength + babysex)^3, data = bw)


bw %>% 
  add_predictions(bw_2plm) %>% 
  add_residuals(bw_2plm) %>% 
  ggplot(aes(x = pred, y = resid)) + geom_point()
